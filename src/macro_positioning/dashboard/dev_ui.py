...
:root {
  ...
  --yellow: #f5b642; --purple: #a679f0; --orange: #ff8642;
}
...
.checklist li { display: flex; gap: 10px; padding: 10px 0;
  border-bottom: 1px solid var(--border); align-items: flex-start; }
.checklist li:last-child { border-bottom: none; }
.icon { flex-shrink: 0; width: 20px; height: 20px; border-radius: 50%;
  display: flex; align-items: center; justify-content: center; font-size: 11px;
  cursor: pointer; margin-top: 1px; user-select: none; transition: transform .1s; }
...
.brain-row { display: grid; grid-template-columns: 90px 110px 100px 1fr 60px;
  align-items: center; padding: 9px 12px; background: var(--surface); border: 1px solid var(--border);
...
function render(ops, cl, brain) {
  ...
  const kpis = [
    ...
    { n: (ops.data_sources.find(s => s.name.includes('FRED'))?.series_count || 0), l: 'FRED Series', c: 'var(--orange)' },
  ...
  items.forEach(t => {
    ...
    li.innerHTML = `
      <span class="icon ${iCls}" title="Click to toggle">${icon}</span>
      <span class="t ${doneCls}">
  ...
  if (brain?.stats) {
    document.getElementById('brain-meta').textContent =
      `${brain.stats.synthesis_calls} synthesis · ${brain.stats.vision_calls} vision · ${brain.stats.avg_latency_ms.toFixed(0)}ms avg · ${(brain.stats.error_rate*100).toFixed(0)}% err`;
  }
  if (!calls.length) {
