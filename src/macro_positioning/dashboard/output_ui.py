...
the macro analysis itself. Clean, focused, mobile-friendly. Designed to
answer "what's the current positioning?" in under 5 seconds.
"""
...
.hero-regime::before { content:''; position:absolute; top:0; right:0; width:280px; height:280px;
...
.trade-rank { font-size:11px; font-weight:700; color: var(--text-muted); min-width:18px; }
...
.theme-card { background: var(--surface); border:1px solid var(--border); border-radius:10px;
  padding:14px 16px; }
...
.asset-chip { font-size:10px; background: rgba(91,156,254,0.08); color: var(--accent);
...
.thesis-theme { font-size:13px; font-weight:600; text-transform:capitalize; min-width:110px; }
...
.thesis-body { padding:0 18px 16px 18px; font-size:13px;
  border-top:1px solid var(--border); max-height:0; overflow:hidden; transition: max-height .25s; }
...
.thesis-list-item { padding:4px 0; color: var(--text); font-size:13px; line-height:1.5; }
...
.risk-card { background: rgba(255,90,95,0.04); border:1px solid rgba(255,90,95,0.22);
  border-radius:10px; padding:14px 16px 14px 40px; position:relative; font-size:13px; line-height:1.55; }
.risk-card::before { content:'⚠'; position:absolute; top:14px; left:14px; color: var(--red); font-size:16px; }
...
function render(cmd, memo, brain) {
  ...
  bc.innerHTML = '<div class="bias-cell bull"><div class="n">' + (cmd.bullish_count||0) + '</div><div class="lbl">Bullish</div></div>'
  ...
  themes.forEach(c => {
    ...
    card.innerHTML = '<div class="theme-name"><span>' + escapeHtml(c.theme) + '</span><span style="font-size:11px;color:var(--text-dim)">' + (c.avg_confidence*100).toFixed(0) + '%</span></div>'
  ...
  theses.forEach((t, i) => {
    ...
    el.className = 'thesis';
    el.innerHTML = '<div class="thesis-head" data-idx="' + i + '">'
      ...
      + '<span class="thesis-conf"><span class="conf-bar"><div style="width:' + conf + '%"></div></span>' + conf + '%</span>'
      + '</div>'
      + '<div class="thesis-body">'
      + '<div class="thesis-meta"><span><b>Horizon:</b> ' + escapeHtml(t.horizon||'—') + '</span><span><b>Status:</b> ' + escapeHtml(t.status||'active') + '</span>'
